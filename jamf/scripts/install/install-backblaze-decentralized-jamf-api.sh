#!/bin/bash
# Backblaze Business Group Install (Jamf Pro) — (Jamf API variant)
#
# Purpose:
#   Installer that enrolls/signs-in devices to a Backblaze Business Group.
#   This variant prefers the real user email from Jamf Pro inventory via API, with an
#   optional explicit email override for controlled testing.
#
# Jamf Script Parameters:
#   $4 = Backblaze Group ID            (required)
#   $5 = Backblaze Group Auth Token    (required)
#   $6 = Backblaze Email override      (optional; explicit email for controlled
#   $7 = Backblaze Region              (optional)
#   $8 = Jamf Pro URL                  (optional; required for API-based email lookup)
#   $9 = Jamf API Client ID            (optional; required for API-based email lookup)
#   $10 = Jamf API Client Secret       (optional; required for API-based email lookup)
...
# - Logs to stdout + /var/log/backblaze_mdm_decentralized_jamf_api.log
...
# LOGGING
#############################################
LOG_FILE="/var/log/backblaze_mdm_decentralized_jamf_api.log"
...
log "=== Backblaze Business Group install started (Jamf API variant) ==="
...
if [[ -n "${8-}" ]]; then JAMF_URL="$8"; fi
if [[ -n "${9-}" ]]; then JAMF_CLIENT_ID="$9"; fi
if [[ -n "${10-}" ]]; then JAMF_CLIENT_SECRET="${10}"; fi
if [[ -n "${11-}" ]]; then BZ_START_BACKUP="$11"; fi

#############################################
# EMAIL RESOLUTION (Jamf API)
#############################################
...
JAMF_URL="$(printf '%s' "$JAMF_URL" | sed 's:/*$::')"

# Prefer explicit email override for controlled.
if [[ -n "${BZ_EMAIL:-}" ]]; then
  EMAIL_SOURCE="explicit override"
else
  # Primary path: look up the assigned user email in Jamf inventory via API.
  if [[ -n "${JAMF_URL:-}" && -n "${JAMF_CLIENT_ID:-}" && -n "${JAMF_CLIENT_SECRET:-}" && -n "${SERIAL_NUMBER:-}" ]]; then
    log "Attempting Jamf API email lookup for serial ${SERIAL_NUMBER}."

    set +e
    JAMF_TOKEN="$(
      curl -s --connect-timeout 15 --max-time 60 -X POST "${JAMF_URL}/api/v1/oauth/token" \
    ...
    if [[ $TOKEN_RC -eq 0 && -n "${JAMF_TOKEN:-}" ]]; then
      set +e
      JAMF_API_EMAIL="$(
        curl -sG --connect-timeout 15 --max-time 60 "${JAMF_URL}/api/v1/computers-inventory" \
      ...
      if [[ $LOOKUP_RC -eq 0 && -n "${JAMF_API_EMAIL:-}" ]]; then
        BZ_EMAIL="$(printf '%s' "$JAMF_API_EMAIL" | tr -d '[:space:]')"
        EMAIL_SOURCE="Jamf inventory email"
        log "Jamf API returned assigned user email: ${BZ_EMAIL}"
        if [[ "$BZ_EMAIL" != *"@"* ]]; then
          log "ERROR: Jamf API returned an invalid email value: '${BZ_EMAIL}'"
          exit 1
        fi
      else
        log "WARN: Jamf API lookup completed but did not return an email."
      fi
    else
      log "WARN: Jamf API token request failed or returned no token."
    fi
  fi
fi
...
if [[ -n "${BZ_EMAIL:-}" ]]; then
  ...
  log "INFO: Backblaze install deferred because Jamf inventory email is not available yet."
  log "  CURRENT_USER='${CURRENT_USER:-}'"
  log "  JAMF_USER_NAME='${JAMF_USER_NAME:-}'"
  log "  SERIAL_NUMBER='${SERIAL_NUMBER:-}'"
  if [[ -n "${JAMF_URL:-}" ]]; then
    log "  JAMF_URL_SET='yes'"
  else
    log "  JAMF_URL_SET='no'"
  fi
fi
...
if [[ -z "$BZ_GROUP_ID" || -z "$BZ_GROUP_TOKEN" ]]; then
  ...
  log "Provide via Jamf parameters $4-$5."
  exit 1
fi

if [[ -z "$BZ_EMAIL" ]]; then
  log "INFO: No Jamf inventory email is available yet, so Backblaze onboarding will be skipped for now."
...
TARGET_USER="${CURRENT_USER:-${JAMF_USER_NAME:-}}"
...
if normalize_bool "${BZ_START_BACKUP:-}"; then
  ...
  log "Start-backup not requested (set Jamf $11 or env BZ_START_BACKUP=1 to enable)."
fi
