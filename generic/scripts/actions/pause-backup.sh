...
# Action: Pause backups (via bzcli)
...
log() {
  local msg="[$(date '+%Y-%m-%d %H:%M:%S')] [backblaze][pause-backup] $*"
...
# Root is recommended; some environments may require it.
if [[ ${EUID} -ne 0 ]]; then
...
if [[ $RC -eq 0 ]]; then
  log "SUCCESS: pause-backup executed."
else
